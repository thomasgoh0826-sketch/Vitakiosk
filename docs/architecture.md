...
FastAPI `/health` exposes a provider summary for development diagnostics only. The React app reads it in dev mode and shows a small local badge such as `AI: ollama`, `STT: faster_whisper`, `Avatar: vrm`, and `VRM: vita-new`. This badge is informational; it does not couple backend provider selection to frontend renderer selection.
