...
- The enlarged Shallow Curve deck must size the active leaflet as the clear hero object while leaving enough safe stage width for real previous/next leaflets to be visibly distinct. Side leaflets should remain clearly visible around 78% of the active leaflet scale when real neighbors exist, with stronger shallow wall treatment around `translateZ(-90px)` and `rotateY(±16deg)` rather than a nearly flat stack or an orbiting carousel.
