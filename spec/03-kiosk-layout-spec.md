...
- The enlarged Shallow Curve deck must size the active leaflet as the clear hero object, using more of the viewer stage than side cards while preserving full artwork containment. Side leaflets should remain visible around 82% of the active leaflet scale when real neighbors exist, with shallow `translateZ` depth and about `rotateY(±10deg)` rather than a flat card or an orbiting carousel.
