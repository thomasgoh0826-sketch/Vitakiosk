...
- Enlarged leaflet previews must use a Slightly Cylindrical / Shallow Curve floating deck pattern. The active leaflet is centered, largest, facing the customer, fully readable, and fully visible; previous and next leaflets remain visible on the left/right when they exist, smaller, dimmer, and gently angled backward with shallow depth. Spherical, circular, orbit, wheel, coverflow spin, full 3D carousel, and hidden single-card replacement carousel motion are not accepted.
