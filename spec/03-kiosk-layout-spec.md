...
- The normal/collapsed promotion region must show exactly one large primary leaflet hero at a time. It must not split into two stacked cards, show multiple visible cards in the right rail, or reveal extra campaign/promotion cards merely because the container is wider or taller.
- The normal/collapsed promotion region shows only the leaflet artwork itself: no title block, branch/source/validity metadata block, or description text below the artwork. Branch/source/validity/description details belong only in the enlarged leaflet viewer.
- The single visible leaflet must fill the available poster frame responsively instead of remaining as a tiny fixed-size card in a large empty panel. Additional active branch-valid leaflets remain browseable through the enlarged swipe/drag viewer, not as multiple main-panel cards.
...
- Enlarged leaflet previews must keep transformed side-preview overflow inside the swipe scene in stacked/narrow layouts so the active leaflet and side previews cannot visually cover the metadata panel.
