...
- The full typing surface includes a real touch keyboard area for EN mode with QWERTY keys, Space, and Backspace as an explicit backup only.
- Bahasa Melayu text is typed in EN mode because it uses the Latin keyboard.
- The kiosk must not provide a Chinese virtual keyboard mode, fake Chinese phrase buttons, or pinyin candidate keyboard. Chinese text entry relies on the device native Chinese IME / pinyin keyboard or an external keyboard through the normal text field.
