...
- The promotion region must be responsive to its own container: narrow/short panels show one large primary leaflet, while wider/taller panels can reveal additional active branch-valid leaflets using a readable grid. A single available leaflet must fill the available poster frame instead of remaining as a tiny fixed-size card in a large empty panel.
