...
- The enlarged moderate cylindrical deck must use distinct visual slots for previous, active, and next leaflets instead of a piled/overlapping stack. The active leaflet remains centered and readable at a larger hero size, while real side leaflets remain clearly visible around 74% of the active leaflet scale and use moderate treatment around `translateZ(-120px)` and `rotateY(±20deg)` without becoming spherical, wheel-like, or buried behind the center card.
- Side leaflet orientation must open away from the center card rather than pinch inward: the previous/left leaflet opens outward to the left, and the next/right leaflet opens outward to the right. In the current CSS perspective this is represented by approximately `rotateY(-20deg)` for previous and `rotateY(20deg)` for next.
...
- During drag and snap, the current active leaflet must not scale above 1, pulse outward, or jump forward in depth; the active card remains visually stable while the deck moves through the moderate curve.
