...
- Leaflet preview modals must render as one clean near-full-screen holographic swipe stage, not a modal box containing a nested carousel box. The active leaflet is the large centered hero object, neighboring leaflets are subtle depth previews when multiple leaflets exist, and touch/mouse/trackpad swipe or drag on the whole stage changes the active leaflet.
- Leaflet preview modals must keep keyboard arrows as an accessibility fallback, Escape closes the modal, clicking outside the leaflet stage closes it, clicking inside the stage or metadata does not close it, and `Previous`/`Next` buttons are not shown as the customer interaction.
- Leaflet preview modals must not show a visible X close button, visible dots, visible `1 / 3` style page indicators, particles, fog, smoke, dust, light-trail elements, noisy atmosphere, or heavy blur/dim overlays. The stage must keep clean safe layout space for the swipe hint and active-leaflet metadata, and must display only existing adapter-supplied leaflet data.
