...
- Leaflet preview modals must render as a clean dark neon holographic gallery: the active leaflet is centered and highlighted, neighboring leaflets are partially visible when multiple leaflets exist, touch/mouse/trackpad swipe or drag changes the active leaflet, keyboard arrows remain an accessibility fallback, Escape closes the modal, clicking outside the leaflet stage closes it, clicking inside the stage does not close it, and `Previous`/`Next` buttons are not shown as the customer interaction.
- Leaflet preview modals must not show a visible X close button, visible dots, visible `1 / 3` style page indicators, particles, fog, smoke, dust, light-trail elements, or heavy blur/dim overlays. The modal must keep safe layout space for the swipe hint and active-leaflet metadata, and must display only existing adapter-supplied leaflet data.
