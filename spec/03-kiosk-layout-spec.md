...
- Enlarged leaflet previews must use a Flat Horizontal Floating Deck pattern. The active leaflet is centered, largest, flat, fully readable, and fully visible; previous and next leaflets remain visible on the left/right when they exist, smaller and dimmer, moving only on a horizontal track. Spherical, cylindrical, orbit, wheel, coverflow spin, and hidden single-card replacement carousel motion are not accepted.
- Flat Horizontal deck corrections must not redesign, move, attach, remove, or restyle the enlarged-view metadata panel. Metadata remains separate from the leaflet deck and must not move with the cards.
- The enlarged Flat Horizontal deck must size the active leaflet as the clear hero object, using more of the viewer stage than side cards while preserving full artwork containment. Side leaflets should remain visible at roughly 70-80% of the active leaflet scale when real neighbors exist, and metadata must not force the active leaflet into a small card.
- Leaflet deck swipe animation must be smooth horizontal motion with controlled snap settling. Bounce, overshoot, pop-forward, elastic spring, aggressive active-card scale-up, translateZ jumps, or card bumping toward the user when it becomes active are not accepted.
- During drag and snap, the current active leaflet must not scale above 1 or pulse outward; the active card remains visually stable while the deck moves horizontally.
...
- Enlarged leaflet previews must keep the active leaflet clearly in front during swipe/drag. Neighboring leaflets stay within the overlay stage as flat left/right panels and must never appear behind or inside the kiosk page background.
