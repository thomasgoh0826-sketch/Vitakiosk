...
describe("typed input layout CSS contract", () => {
  ...
  it("keeps the enlarged leaflet viewer as a foreground overlay with metadata outside the hero card", () => {
    ...
    expect(normalizedStyles).toMatch(/\.floating-leaflet-panel\s*\{[^}]*width: clamp\(340px, 28dvw, 420px\);[^}]*height: min\(100%, 78dvh\);/);
    ...
    expect(normalizedStyles).toMatch(/\.floating-leaflet-panel\.is-neighbor\s*\{[^}]*opacity: var\(--leaflet-deck-opacity, 0\.9\);/);
    expect(normalizedStyles).toMatch(
    ...
    expect(normalizedStyles).toMatch(
      /@media \(max-width: 1120px\), \(max-height: 740px\)\s*\{[\s\S]*\.floating-leaflet-panel\s*\{[\s\S]*width: clamp\(320px, 33dvw, 360px\);[\s\S]*height: min\(100%, 74dvh\);/,
    );
    expect(normalizedStyles).toMatch(
  ...
  it("uses a holographic product transform instead of a cheap 180-degree flip", () => {
    ...
    expect(normalizedStyles).toMatch(/html\.product-expanded,\s*body\.product-expanded\s*\{[^}]*overflow: hidden;/);
    expect(normalizedStyles).toMatch(/\.product-viewer-backdrop\s*\{[^}]*position: fixed;[^}]*z-index: 890;[^}]*overflow: hidden;/);
    expect(normalizedStyles).toMatch(/\.product-viewer-backdrop\s*\{[^}]*overscroll-behavior: none;/);
    expect(normalizedStyles).toMatch(/\.product-viewer-stage\s*\{[^}]*width: min\(980px, calc\(100dvw - clamp\(36px, 8dvw, 112px\)\)\);/);
    expect(normalizedStyles).toMatch(/\.product-viewer-stage\s*\{[^}]*box-sizing: border-box;/);
    expect(normalizedStyles).toMatch(/\.product-viewer-stage\s*\{[^}]*overflow-x: hidden;[^}]*overflow-y: auto;/);
    expect(normalizedStyles).toMatch(/\.product-viewer-stage\s*\{[^}]*overscroll-behavior: contain;/);
    ...
    expect(normalizedStyles).toMatch(/\.product-viewer-stage::before\s*\{[^}]*background-size: 220% 100%;/);
    expect(normalizedStyles).toMatch(/@keyframes product-viewer-sweep\s*\{[^}]*background-position:/);
    expect(normalizedStyles).not.toMatch(/@keyframes product-viewer-sweep\s*\{[^}]*transform:/);
