...
| Frontend build | `npm.cmd run build --prefix frontend` | TypeScript and Vite production build completed; default bundle 335.83kB JS before gzip, optional lazy ThreeAvatarRenderer chunk 80.26kB, VrmAvatarRenderer chunk 193.86kB, AvatarModel chunk 861.93kB, CSS bundle 56.71kB, and local VRM asset 15,369.46kB | Pass |
...
| Avatar | Current direction requires a futuristic AI assistant screen with a larger full-body/near-full-body VRM display chamber | Render uses a self-hosted VRM avatar in a dark holographic chamber with head, arms, and lower body visible, plus scanning glow, waveform, and state label | Matched |
