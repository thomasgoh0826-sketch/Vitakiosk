...
| Backend tests | `python -m pytest backend/tests -v` | 65 passed | Pass |
| Controlled provider config | Included in `python -m pytest backend/tests -v` | 6 tests passed: provider selectors default to mock, credentials do not auto-enable live providers, OpenAI Whisper STT requires explicit selector, missing `OPENAI_API_KEY` fails closed, and invalid selectors fail closed | Pass |
| Frontend tests | `npm.cmd run test:run --prefix frontend` | 14 files, 76 tests passed | Pass |
...
| Frontend build | `npm.cmd run build --prefix frontend` | TypeScript and Vite production build completed; default bundle 345.34kB JS before gzip, optional lazy ThreeAvatarRenderer chunk 80.26kB, VrmAvatarRenderer chunk 194.03kB, AvatarModel chunk 862.26kB, CSS bundle 62.19kB, local VRM asset 15,369.46kB, and alternate local VRM asset 16,532.72kB | Pass |
