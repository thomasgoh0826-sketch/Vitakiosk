...
| VRM pose and portrait runtime QA | In-app Browser / local Chrome at 1024×768 with `VITE_AVATAR_RENDERER=vrm`; screenshot review `reports/evidence/vrm-avatar-portrait-stage-1024x768.png` | Runtime evidence shows self-hosted VRM model active in a vertical portrait stage, no circular hologram mask, face/head/shoulders/upper torso visible, skirt not dominant, and no horizontally stretched T-pose arms | Pass |
...
| Frontend build | `npm.cmd run build --prefix frontend` | TypeScript and Vite production build completed; default bundle 335.79kB JS before gzip, optional lazy ThreeAvatarRenderer chunk 80.26kB, VrmAvatarRenderer chunk 194.21kB, AvatarModel chunk 861.88kB, and local VRM asset 15,369.46kB | Pass |
...
VRM portrait-stage screenshot: [1024x768 VRM assistant portrait stage](evidence/vrm-avatar-portrait-stage-1024x768.png).
